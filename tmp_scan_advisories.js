...
for (const line of lines) {
  if (line === "[[package]]") {
    ...
    cur.name = line.split("=")[1].trim().slice(1, -1);
  } else if (cur && line.startsWith("version = ")) {
    cur.version = line.split("=")[1].trim().slice(1, -1);
...
pkgs.forEach((p) => {
  if (!versionsByName[p.name]) versionsByName[p.name] = new Set();
  versionsByName[p.name].add(p.version);
});
const packageVersions = {};
Object.keys(versionsByName).forEach((name) => {
  if (pkgNames.includes(name))
    packageVersions[name] = [...versionsByName[name]];
});
const semver = {
  parse: (v) => {
  ...
  cmp: (a, b) => a.major - b.major || a.minor - b.minor || a.patch - b.patch,
...
function satisfiable(version, range) {
  ...
  if (range.includes("||"))
    return range
      .split("||")
      .map((r) => r.trim())
      .some((r) => satisfiable(version, r));
  if (range.includes(","))
    return range
      .split(",")
      .map((r) => r.trim())
      .every((r) => satisfiable(version, r));
  if (range.startsWith(">=")) {
    const other = semver.parse(range.slice(2).trim());
    return semver.cmp(v, other) >= 0;
  }
  if (range.startsWith(">")) {
    const other = semver.parse(range.slice(1).trim());
    return semver.cmp(v, other) > 0;
  }
  if (range.startsWith("<=")) {
    const other = semver.parse(range.slice(2).trim());
  ...
  for (const pkg of Object.keys(packageVersions)) {
    ...
    for (const path of relevant) {
      ...
      for (const version of packageVersions[pkg]) {
        const isPatched =
          patched.length && patched.some((r) => satisfiable(version, r));
        const isUnaffected =
          unaffected.length && unaffected.some((r) => satisfiable(version, r));
